...
#This code defines a function called count that takes a list as an argument and returns the number of integers in the list.
def count(lst):
    num=0
    for i in lst:
...
#This function takes a list and returns the sum of all the numerical elements in the list.
def add(lst):
    plus=0
    for i in lst:
        try:
...
#This function returns a boolean value indicating whether a particular attribute appears in the file at the specified position.
...
#assigns course codes to streams
def choice(stream):
    if stream.lower()=='cse' or stream.lower()=='cseai' or stream.lower()=='cseaiml' or stream.lower()=='cseiotcsbs':
...
#defines a function get_batch which reads a file Batch.csv , creates a list of all the rows in file ,
#creates a 'column' list containing the 1st element of each row except the first row , and reurns the 'column' list
def get_batch():
...
# defines a funtion that removes that removes a chosen row from the Student.csv file
def remove(string):
    with open(f'C:/PythonProgrammingProject_main-folder/Student.csv','r+',newline='') as f:
        ...
        f.truncate()       #deletes contents of the file
        writer=csv.writer(f)
...
#defining the function that plots the course graph
def course_graph():
...
#defining the the function that plots the batch graph
def batch_graph(arg):
...
#defines the function plotting the department graph
...
try:
    os.makedirs(f'{path}/ReportCards') #creates a directory called "ReportCards" in the specified path
    message=True
except:
    message=False
...
for i in range(student_no):
    ...
    batch_id=stream+batch[2:4]    #creating the batch id
    student_id=batch_id+roll      #creting the student id
    batch_name=stream+batch

    if duplicate('Student.csv',student_id,0): #checks if student details are alredy in the directory
    ...
    else: #takes user input of marks
        print()
